...
const SearchAgarbatiPrimary = async (req: Request, res: Response) => {
    try {
        ...
        let rcnEntries
        if(limit===0 && offset===0){
             rcnEntries = await agarbatiPrimaryEntryModel.findAll({
                where,
        ...
        return res.status(200).json({ msg: 'Agarbati Entry found', rcnEntries })
    }
    catch (err) {
        console.log(err)
...
export default SearchAgarbatiPrimary