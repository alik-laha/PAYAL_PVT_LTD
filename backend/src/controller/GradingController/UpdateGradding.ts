...
const UpdateGradding = async (req: Request, res: Response) => {
    try {
        ...
        if (RcnGradingData) {
            const RcnGradingEditData = await RcnGradingEdit.create({ id, date, A, B, C, D, E, F, G, dust, Mc_on, Mc_off, Mc_breakdown: Mc_breakdown, noOfEmployees, grading_lotNo, Mc_name, origin, otherTime, Mc_runTime, feeledBy: feeledBy, editStatus })
            
            
            if (RcnGradingEditData) {
                const data = await WhatsappMsg("RCN Grading Entry", feeledBy,"modify_request")
                console.log(data)
