...
const RCNBigTaihoReCreateForm = (props:Props) => {
    //console.log(props)
    ...
    const handleSubmit2 = async (e: React.FormEvent) => {
        ...
        if(
         ...
         ((props.borma[0].rcv_village ? Number(props.borma[0].rcv_village):0) < Number(rows[0].rcv_village)) ||
            ((props.borma[0].rcv_dpds ? Number(props.borma[0].rcv_dpds):0) < Number(rows[0].rcv_dpds)) ||
            ((props.borma[0].rcv_mayur ? Number(props.borma[0].rcv_mayur):0) < Number(rows[0].rcv_mayur)) ||
            ((props.borma[0].rcv_hamsa ? Number(props.borma[0].rcv_hamsa):0) < Number(rows[0].rcv_hamsa)) ||
            ((props.borma[0].rcv_lw ? Number(props.borma[0].rcv_lw):0) < Number(rows[0].rcv_lw)) ||
            ((props.borma[0].rcv_wholes ? Number(props.borma[0].rcv_wholes):0) < Number(rows[0].rcv_wholes)) 
            || (Number(props.borma[0].rcv_peeling) < Number(rows[0].rcv_peeling))
        ...
        //const operator = operatorRef.current?.value
            ...
            try {
                const initialhumid = await axios.post('/api/bigTaiho/createReissueBigTaiho', { linehumid:formData,
                    LotNo:props.borma[0].LotNo
                 })
               ...
               <Input className="w-2/4 font-semibold text-center bg-yellow-100" placeholder="Date" value={props.scoop[0].LotNo} readOnly /> </div> */}
                ...
                <Input className="w-2/4 justify-center" placeholder="Date" ref={DateRef} type="date" required /> </div>
                   ...
                   <TableHeader className="bg-neutral-100 text-stone-950 ">
                    ...
                    <TableHead className="text-center">Previous Whole</TableHead>
                    <TableHead className="text-center">Current Whole</TableHead>
                            ...
                            rows.map(( row:BigTaihoRowData,idx:number) => {
                                ...
                                return (
                                    <TableRow key={idx} className="boiling-row-height-scoop">
                                        ...
                                        <TableCell className="text-center"> <Input  type="number" value={row.rcv_peeling} placeholder="Pr." onChange={(e) => handleRowChange(idx, 'rcv_peeling', e.target.value)} required /></TableCell>
                                        ...
                                        <TableCell className="text-center"> <Input  type="number" value={row.rcv_mayur} placeholder="Pr." onChange={(e) => handleRowChange(idx, 'rcv_mayur', e.target.value)} required /></TableCell>
                                        ...
                                        <TableCell className="text-center"> <Input  type="number" value={row.rcv_hamsa} placeholder="Pr." onChange={(e) => handleRowChange(idx, 'rcv_hamsa', e.target.value)} required /></TableCell>
                                        ...
                                        <TableCell className="text-center"> <Input  type="number" value={row.rcv_wholes} placeholder="Pr." onChange={(e) => handleRowChange(idx, 'rcv_wholes', e.target.value)} required /></TableCell>
...
export default RCNBigTaihoReCreateForm;
