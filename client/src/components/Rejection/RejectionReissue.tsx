...
const RejectionReCreateForm = (props: Props) => {
    //console.log(props)
    ...
    const handleSubmit2 = async (e: React.FormEvent) => {
        ...
        if (
            Number(rows[0].rcv_opening) !== (
                ...
                Number(rows[0].rcv_bigTaiho) + Number(rows[0].rcv_village) 
            )
        ) {
        ...
        if (
            ...
            Number(props.borma[0].issue_add_8) < Number(rows[0].rcv_mayur) ||
            Number(props.borma[0].rcv_village) < Number(rows[0].rcv_village) ||
        ...
        try {
            const initialhumid = await axios.post('/api/rejection/createReissueRejection', {
                linehumid: formData,
                LotNo: props.borma[0].LotNo
            })
    ...
    const handleOpeningChange = (index:number,e: React.ChangeEvent<HTMLInputElement>) => {
       ...
       useEffect(() => { 
                ...
                if(rows[0]){
                    ...
                    rows[0].rcv_villageN = ((rows[0].rcv_village ? Number(rows[0].rcv_village) : 0) * ((100 - Number(rows[0].issue_add_3)) / 100));
            ...
            <div className="px-5 py-2 overflow-auto">
               ...
               <Input className="w-2/4 font-semibold text-center bg-yellow-100" placeholder="Date" value={props.scoop[0].LotNo} readOnly /> </div> */}
                    ...
                    <div className="my-2 text-sm flex font-semibold text-red-600 text-justify ">
                      * Current [ Peeling + Mayur + DPDS + Sorting + Wholes + LW + BigTaiho + Village ] should be equal to {props.borma[0].current_backlog} Kg
                  </div>
...
export default RejectionReCreateForm;
