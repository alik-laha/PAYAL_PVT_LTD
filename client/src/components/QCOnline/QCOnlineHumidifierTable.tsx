...
const QCOnlineHumidifierTable = () => {
  ...
  const handleSearch = async () => {
    const response = await axios.post('/api/qconline/searchQCOnlineHumidifier', {
      fromDate: fromdate,
      toDate: todate,
    }, { params: { page, limit } })
  ...
  const exportToExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Humidifier_Report');
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], { type: 'application/octet-stream' });
    saveAs(blob, 'QC_Humidifier_Report_' + currDate + '.xlsx');
  }
...
export default QCOnlineHumidifierTable;
