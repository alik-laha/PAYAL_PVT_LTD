...
const QCOnlineNanopix = () => {
  const [errortext, setErrorText] = useState<string>("");
  const [isdisable, setIsDisable] = useState<boolean>(false);

  const gradingCountRef = useRef<HTMLInputElement>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      gradingCount: gradingCountRef.current?.value,
      date,
      time,
      cleaningStatus,
      cleanRemarks:
    ...
    axios
      .post("/api/qconline/createQCOnlineNanopix", payload)
      .then(() => {
        if (successdialog) successdialog.showModal();
        // reset
        gradingCountRef.current!.value = "";
        setCupCleaningStatus("");
        setMagicCleaningStatus("");
        setCleaningStatus("");
      ...
      <dialog id="successDialog" className="dashboard-modal">
        ...
        <span className="flex">
          ...
          <p className="pl-3 mt-1 font-medium">
            QC Online Nanopix Reported successfully!
          </p>
        </span>
      </dialog>
...
export default QCOnlineNanopix;
