...
const QCOnlineGradingTable = () => {
  ...
  const handleSearch = async () => {
    const response = await axios.post(
      "/api/qconline/searchQCOnlineGrading",
      {
        fromDate: fromdate,
        toDate: todate,
      },
  ...
  const exportToExcel = async () => {
    const response = await axios.post("/api/qconline/searchQCOnlineGrading", {
      fromDate: fromdate,
      toDate: todate,
    });
    const data = await response.data;
      ...
      <Table className="mt-4">
        ...
        <TableBody>
          {ItemWiseData.length > 0 ? (
              ...
              <TableRow key={item.id}>
                ...
                <TableCell className="text-center">
                  {item.vibratorspeed2}
                   ...
                   <TableCell className="text-center"> {item.cleaningStatus === "OK" ? (
                                                                            ...
                                                                            ) : item.cleaningStatus)}</TableCell>
                                <TableCell className="text-center"> {item.maintainance === "OK" ? (
                                                                            <p className="flex flex-row justify-center">
                                                                                                       <SiTicktick color="green" size={18} />
                                                                                                     </p>
                                                                        ) : (
            ...
            <TableRow>
              <TableCell colSpan={12} className="text-center text-red-500 py-6">
                No Result
              </TableCell>
            </TableRow>
          )}
...
export default QCOnlineGradingTable;
