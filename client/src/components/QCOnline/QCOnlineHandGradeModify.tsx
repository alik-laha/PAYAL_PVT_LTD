...
const QCeditHandGrade = ({ data }: Props) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    axios.put(`/api/qconline/editQCOnlineHandGrade/${data.id}`, payload)
      .then(() => {
  ...
  return (
    <>
      <div className="pb-6 px-6">
        <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
          <div className="flex mt-1">
            ...
            <Input className="w-2/4 text-center justify-center bg-yellow-100"
              value={date} type="date" onChange={(e) => setDate(e.target.value)} required />
          </div>

          <div className="flex mt-1">
            <Label className="w-2/4 pt-1">Time</Label>
            <Input className="w-2/4 text-center justify-center bg-yellow-100"
              value={time} type="time" onChange={(e) => setTime(e.target.value)} required />
          </div>

          {/* Lot No */}
          <div className="flex">
            <Label className="w-2/4 pt-1">Lot No</Label>
            <Input className="w-2/4 text-center" value={lotNo} onChange={(e) => setLotNo(e.target.value)} type="text" required />
          </div>
          ...
          <ScrollArea className="max-h-24 overflow-scroll dropdown-content w-30"
            ...
            {gradeData.map((item: any) => (
              <div key={item.id}
                ...
                <p className="font-medium text-sm text-blue-900 py-1">{item.sku}</p>
              </div>
            ))}
          </ScrollArea>
          ...
          <div className="flex">
            ...
            <Input className="w-2/4 text-center"
      ...
      <dialog id="successDialog" className="dashboard-modal">
        <button className="dashboard-modal-close-btn" onClick={() => { successdialog?.close(); window.location.reload() }}>X</button>
        <span className="flex">
          <img src={tick} height={25} width={25} alt="success" />
          <p className="pl-3 mt-1 font-medium">QC Online Hand Grade updated successfully!</p>
        </span>
      </dialog>
...
export default QCeditHandGrade
