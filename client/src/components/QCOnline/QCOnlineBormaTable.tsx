...
const QCOnlineBormaTable = () => {
  ...
  const handleSearch = async () => {
    const response = await axios.post('/api/qconline/searchQCOnlineBorma', {
      fromDate: fromdate,
      toDate: todate,
    }, { params: { page, limit } })
  ...
  const exportToExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Borma_Report');
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([wbout], { type: 'application/octet-stream' });
    saveAs(blob, 'QC_Borma_Report_' + currDate + '.xlsx');
  }
    ...
    <div className="ml-5 mt-5 ">
      ...
      <Table className="mt-4">
        <TableHeader className="bg-neutral-100 text-stone-950 ">
          ...
          <TableHead className="text-center">Borma MC No</TableHead>
          <TableHead className="text-center">Pressure</TableHead>
       
            
          <TableHead className="text-center">NW Quality</TableHead>
          <TableHead className="text-center">Trolley Burn Quality</TableHead>
            ...
            ItemWiseData.length > 0 ? (ItemWiseData.map((item: any, idx: number) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-center text-cyan-500 font-semibold">{item.Origin}</TableCell>
                     <TableCell className="text-center">{item.bormaNo}</TableCell>
                ...
                <TableCell className="text-center">
                  {item.nwQuality === 'OK'
                    ...
                    : <button className="bg-red-500 p-2 text-white rounded fix-button-width">{item.nwQuality}</button>}
                </TableCell>
                <TableCell className="text-center">
                  {item.burnQuality === 'OK'
                    ? <button className="p-2 text-white rounded bg-green-500 fix-button-width">OK</button>
                    : <button className="bg-red-500 p-2 text-white rounded fix-button-width">{item.burnQuality}</button>}
                </TableCell>
                <TableCell className="text-center">
              ...
              <TableRow>
                <TableCell colSpan={15} className="text-center text-red-500 py-6">
                  No Result
                </TableCell>
              </TableRow>
            )
...
export default QCOnlineBormaTable;
