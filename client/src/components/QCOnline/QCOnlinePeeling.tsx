...
const QCOnlinePeeling = () => {
  const [errortext, setErrorText] = useState<string>("")
  const [isdisable, setIsDisable] = useState<boolean>(false)

  const pressureRef = useRef<HTMLInputElement>(null)
  const peelingTimeRef = useRef<HTMLInputElement>(null)
  const unpeelPcntngRef = useRef<HTMLInputElement>(null)
  const cashewPcntngRef = useRef<HTMLInputElement>(null)
  const peelingQtyRef = useRef<HTMLInputElement>(null)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      peelingTime: peelingTimeRef.current?.value,
      unpeelPcntng: unpeelPcntngRef.current?.value,
      cashewPcntng: cashewPcntngRef.current?.value,
      peelingQty: peelingQtyRef.current?.value,
      date,
      time,
      cleaningStatus,
    ...
    axios.post("/api/qconline/createQCOnlinePeeling", payload)
      .then(() => {
        if (successdialog) successdialog.showModal()
        // reset form
        pressureRef.current!.value = ""
        peelingTimeRef.current!.value = ""
        unpeelPcntngRef.current!.value = ""
        cashewPcntngRef.current!.value = ""
        peelingQtyRef.current!.value = ""
        setCleaningStatus("")
    ...
    <>
      <div className="pb-6 px-6">
        <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
          ...
          <div className="flex">
            ...
            <Input className="w-2/4 text-center" ref={pressureRef} type="number" step="0.01" required placeholder="Enter Pressure" />
          </div>

          {/* Peeling Time */}
          <div className="flex">
            <Label className="w-2/4 pt-1">Peeling Time</Label>
            <Input className="w-2/4 text-center" ref={peelingTimeRef} type="text" required placeholder="Enter Peeling Time" />
          </div>

            {/* Peeling Qty */}
          <div className="flex">
            <Label className="w-2/4 pt-1">Peeling MC Input Quantity</Label>
            <Input className="w-2/4 text-center" ref={peelingQtyRef} type="number" step="0.01" required placeholder="Enter Qty" />
          </div>

          {/* Unpeel % */}
          <div className="flex">
            <Label className="w-2/4 pt-1">Unpeel % in 250 gm</Label>
            <Input className="w-2/4 text-center" ref={unpeelPcntngRef} type="number" step="0.01" required placeholder="%" />
          </div>
      ...
      <dialog id="successDialog" className="dashboard-modal">
        ...
        <span className="flex">
          ...
          <p className="pl-3 mt-1 font-medium">QC Online Peeling Reported successfully!</p>
        </span>
      </dialog>
...
export default QCOnlinePeeling
