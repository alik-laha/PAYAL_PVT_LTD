...
const OilMillPrimaryEntryForm = (props:Props) => {
    ...
    const [rows,setRows]=useState<SectionRowData[]>([{type:'',quantity:0,totalWt:0,totalBill:0}
    ]);
    ...
    const addRow2 = () => {
        setRows([...rows,{type:'',quantity:0,totalWt:0,totalBill:0}])
    }
        ...
        try {
            if (formData.length === 1) {
                for (var data of formData) {
                    await axios.put(`/api/oilMill/updateRcvOilMill/${id}`, { data })
                    await axios.post("/api/gatepass/updateRcvDisptchStatus", {
                        gatePassNo: gatepass,
                        section: 'OilMill'
                    })
                    setErrorText('OilMill Items Dispatched Successfully')
                    if (successdialog) {
                ...
                await axios.put(`/api/oilMill/updateRcvOilMillEntire/${id}`, { formData })
                await axios.post("/api/gatepass/updateRcvDisptchStatus", {
                    gatePassNo: gatepass,
                    section: 'OilMill'
                })
...
export default OilMillPrimaryEntryForm