...
const CreditNoteInitial = (props: any) => {
    const [rcnData, setrcnData]  = useState<[]>([])

    //let scoopdata:ScoopData[]=[]
    console.log(props)
    const handleLineEntry = async (gatePassNo:string) => {
        axios.get(`/api/creditNote/getCreditNoteByGatePass/${gatePassNo}`).then(res=>{
           console.log(res)
                                        ...
                                        <TableCell className="text-center">
                                            <Dialog>
                                              ...
                                              <DialogContent style={{display:'block'}} className='max-w-screen'>
                                                    <DialogHeader>
                                                        <DialogTitle><p className='text-lg text-gray-600 text-center my-3 tracking-wider drop-shadow-xl font-bold'>Credit Note Receiving</p></DialogTitle>
...
export default CreditNoteInitial