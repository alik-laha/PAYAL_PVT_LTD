...
const [remarks, setRemarks] = useState<string>("")
  ...
  if (props.data) {
    ...
    setEditStatus(props.data.editStatus)
    setType(props.data.type)
    setCreatedBy(props.data.createdBy)
    setApprovedBy(props.data.approvedBy)
...
focus-visible:ring-offset-0.5 disabled:cursor-not-allowed disabled:opacity-50" onChange={(e) => setType(e.target.value)}
                ...
                <div className="flex"><Label className="w-2/4 mt-2">Credit Note No.</Label>
                    <Input className="w-2/4 text-center" placeholder="Credit Note No" value={creditNoteNo} onChange={(e) => setCreditNoteNo(e.target.value)} /> </div>
   
                    
                <div className="flex"><Label className="w-2/4 mt-2" >Weight</Label>
                    <Input className="w-2/4 text-center justify-center" placeholder="Row Weight" value={totalWt} onChange={(e) => setTotalWt(e.target.value)} /> </div> 
          
             
                
                <div className="flex">
                    <Label className="w-2/4 mt-2">Qty/Bag Count</Label>
                    <Input className="w-2/4 text-center " placeholder="Qty" type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} />
                </div>
                <div className="flex">
                    <Label className="w-2/4 mt-2">Unit Price</Label>
                    <Input className="w-2/4 text-center " placeholder="Unit Price" type="number" value={unitPrice} onChange={(e) => setUnitPrice(e.target.value)} />
                </div>
                <div className="flex">
                    <Label className="w-2/4 mt-2">Bill Amount</Label>
                    <Input className="w-2/4 text-center " placeholder="Total Bill Amount" type="number" value={totalBill} onChange={(e) => setTotalBill(e.target.value)} />
                </div>
