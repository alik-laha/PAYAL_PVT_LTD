...
const RCNMayurReMix = (props:Props) => {
        ...
        const handleMix = async () => {
            ...
            if((Number(fsourcercv_wholespeel) < 0) || (Number(fsourcercv_wholesunpeel) < 0) || (Number(fsourcercv_DPDS) < 0) || 
                ...
                setisdisable(true)
                try {
                    const initialhumid = await axios.post('/api/mayur/createMixMayur', {destid,destlot,destorigin,destbacklog,destrcv_DPDS,destrcv_sorting,destrcv_village,destrcv_wholespeel,destrcv_wholesunpeel,
                        fsourceid:props.borma.id,fsourcelot:props.borma.LotNo,fsourceorigin:props.borma.origin,fsourcebacklog,fsourcercv_DPDS,fsourcercv_sorting,fsourcercv_village,fsourcercv_wholespeel,fsourcercv_wholesunpeel,
                        amount:(sourcercv_wholesunpeel+sourcercv_wholespeel+sourcercv_DPDS+sourcercv_sorting+
                        sourcercv_village).toFixed(2),bsourcebacklog:props.borma.current_backlog,bdestbacklog:datarcv.current_backlog
                     })
