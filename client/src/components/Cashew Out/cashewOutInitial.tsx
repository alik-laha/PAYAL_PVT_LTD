...
import CashewOutEntryForm from "./cashewOutCreateForm";
...
const CashewOutInitialForm = (props: any) => {
    const [rcnData, setrcnData]  = useState<CashewOutEntryData[]>([])

    //let scoopdata:ScoopData[]=[]
    console.log(props)
    const handleLineEntry = async (gatePassNo:string) => {
        axios.get(`/api/cashewOut/getCashewOutByGatePass/${gatePassNo}`).then(res=>{
           console.log(res)
                                    ...
                                    <TableRow key={idx}>
                                        ...
                                        <TableCell className="text-center">
                                           ...
                                           <DialogContent style={{display:'block'}} className='max-w-screen'>
                                                    <DialogHeader>
                                                        <DialogTitle><p className='text-1xl text-center mt-1'>Cashew Dispatch</p></DialogTitle>

                                                    </DialogHeader>
                                                <CashewOutEntryForm rcn={rcnData}/>
...
export default CashewOutInitialForm