...
import GatepassApproveFinal from "./gatePassApproveFinal";
interface pdfprops {
  data: GatePassData;
}
...
const GatePassTable = () => {
  ...
  const handleGateCanel = (data: GatePassData) => {
    
    if (!cancelremark) {
      setErrView('block')
      return
    }
    axios.put(`/api/gatepass/cancelRemark/${data.id}`, {
      remarks: cancelremark, section: data.section,
      type: data.type, gatepassNo: data.gatePassNo
    }).then((res) => {
      setCancelremark('')
      setErrorText(res.data.message);
      console.log(res.data)
  ...
  const handleNetWeight = (data: GatePassData) => {
    ...
    axios.put(`/api/gatepass/updateNetWeight/${data.id}`, {
      netWeight: netWeight, section: data.section,
      type: data.type, gatepassNo: data.gatePassNo
    }).then((res) => {
      setNetWeight(0)
      setErrorText(res.data.message);
    ...
    ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1'
        ...
        <Table className="mt-4">
          <TableHeader className="bg-neutral-100 text-stone-950 ">
            ...
            {Role !== 'Security' && <TableHead className="text-center" >Admin</TableHead>}
          </TableHeader>
          <TableBody>
              ...
              return (
                <TableRow key={item.id}>
                  ...
                  {Role !== 'Security' && <TableCell className="text-center">
                    ...
                    <PopoverContent className="flex flex-col w-30 text-sm font-medium">
                    {item.status !== 'Closed' && <AlertDialog>
                          <AlertDialogTrigger className="flex">
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                <Input placeholder="Cancel Remark" className='mt-3 w-100 text-center justify-center items-center' value={cancelremark} onChange={(e) => setCancelremark(Number(e.target.value))} required />
